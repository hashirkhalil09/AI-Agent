...
router.post("/generate", async (req, res) => {
  try {
    ...
    if (publishNow) {
      ...
      return res.json({ ...result, caption: post.caption, imageUrl: post.imageUrl });
    }

    res.status(201).json(post);
  } catch (err) {
    console.error("POST /api/posts/generate failed:", err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post("/:id/publish", async (req, res) => {
  try {
    ...
    console.error("POST /api/posts/:id/publish failed:", err.message);
    res.status(500).json({ error: err.message });
  }
});
